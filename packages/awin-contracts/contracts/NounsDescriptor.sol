...
contract awinDescriptor is IawinDescriptor, Ownable {
    ...
    /**
     * @notice Given a token ID and seed, construct a token URI for an official awin DAO noun.
     * @dev The returned value may be a base64 encoded data URI or an API URL.
     */
    function tokenURI(uint256 tokenId, IawinSeeder.Seed memory seed) external view override returns (string memory) {
        if (isDataURIEnabled) {
    ...
    /**
     * @notice Given a token ID and seed, construct a base64 encoded data URI for an official awin DAO noun.
     */
    function dataURI(uint256 tokenId, IawinSeeder.Seed memory seed) public view override returns (string memory) {
        string memory nounId = tokenId.toString();
        string memory name = string(abi.encodePacked('Noun ', nounId));
        string memory description = string(abi.encodePacked('Noun ', nounId, ' is a member of the awin DAO'));
     ...
     */
    function generateSVGImage(IawinSeeder.Seed memory seed) external view override returns (string memory) {
     ...
     */
    function _getPartsForSeed(IawinSeeder.Seed memory seed) internal view returns (bytes[] memory) {
