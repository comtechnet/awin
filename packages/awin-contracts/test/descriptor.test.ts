...
import { awinDescriptor } from '../typechain';
import ImageData from '../files/image-data.json';
import { LongestPart } from './types';
import { deployawinDescriptor, populateDescriptor } from './utils';
...
describe('awinDescriptor', () => {
  let awinDescriptor: awinDescriptor;
  ...
  before(async () => {
    awinDescriptor = await deployawinDescriptor();
    ...
    await populateDescriptor(awinDescriptor);
  });
  ...
  it('should generate valid token uri metadata when data uris are disabled', async () => {
    const BASE_URI = 'https://api.awin.wtf/metadata/';

    await awinDescriptor.setBaseURI(BASE_URI);
    await awinDescriptor.toggleDataURIEnabled();

    const tokenUri = await awinDescriptor.tokenURI(0, {
      background: 0,
  ...
  it('should generate valid token uri metadata when data uris are enabled', async () => {
    ...
    expect(description).to.equal('Noun 0 is a member of the awin DAO');
  ...
  it.skip('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      const tokenUri = await awinDescriptor.tokenURI(i, {
      ...
      expect(description).to.equal(`Noun ${i} is a member of the awin DAO`);
      expect(image).to.not.be.undefined;
