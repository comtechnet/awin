...
import { awinDescriptorFactory, awinToken } from '../typechain';
import { deployawinToken, populateDescriptor } from './utils';
...
describe('awinToken', () => {
  ...
  it('should allow the minter to mint a noun to itself and a reward noun to the noundersDAO', async () => {
    const receipt = await (await awinToken.mint()).wait();

    const [, , , noundersNounCreated, , , , ownersNounCreated] = receipt.events || [];

    expect(await awinToken.ownerOf(0)).to.eq(noundersDAO.address);
    ...
    expect(await awinToken.ownerOf(1)).to.eq(deployer.address);
  ...
  it('should set symbol', async () => {
    expect(await awinToken.symbol()).to.eq('NOUN');
  });

  it('should set name', async () => {
    expect(await awinToken.name()).to.eq('awin');
  });

  it('should allow minter to mint a noun to itself', async () => {
    await (await awinToken.mint()).wait();

    const receipt = await (await awinToken.mint()).wait();
    const nounCreated = receipt.events?.[3];

    expect(await awinToken.ownerOf(2)).to.eq(deployer.address);
  ...
  it('should emit two transfer logs on mint', async () => {
    ...
    await (await awinToken.setMinter(minter.address)).wait();
    await (await awinToken.transferOwnership(creator.address)).wait();

    const tx = awinToken.connect(minter).mint();

    await expect(tx)
      .to.emit(awinToken, 'Transfer')
      .withArgs(constants.AddressZero, creator.address, 2);
    await expect(tx).to.emit(awinToken, 'Transfer').withArgs(creator.address, minter.address, 2);
  });

  it('should allow minter to burn a noun', async () => {
    await (await awinToken.mint()).wait();

    const tx = awinToken.burn(0);
    await expect(tx).to.emit(awinToken, 'NounBurned').withArgs(0);
  });

  it('should revert on non-minter mint', async () => {
    const account0AsNounErc721Account = awinToken.connect(noundersDAO);
    await expect(account0AsNounErc721Account.mint()).to.be.reverted;
  });

  describe('contractURI', async () => {
    it('should return correct contractURI', async () => {
      expect(await awinToken.contractURI()).to.eq(
        'ipfs://QmZi1n79FqWt2tTLwCqiy6nLM6xLGRsEPQ5JmReJQKNNzX',
      );
    });
    it('should allow owner to set contractURI', async () => {
      await awinToken.setContractURIHash('ABC123');
      expect(await awinToken.contractURI()).to.eq('ipfs://ABC123');
    });
    it('should not allow non owner to set contractURI', async () => {
      const [, nonOwner] = await ethers.getSigners();
      await expect(awinToken.connect(nonOwner).setContractURIHash('BAD')).to.be.revertedWith(
